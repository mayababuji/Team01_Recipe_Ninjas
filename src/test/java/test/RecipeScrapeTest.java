...
public class RecipeScrapeTest extends BaseTest {
	...
	public void scrapeAndInsertFilteredRecipes() {
		...
		for (String foodCategory : foodCategories) {
			...
			for (String recipeLink : recipeLinks) {
				...
				if (lfvPass) {
					...
					List<String> lfvUpdatedIngredients = recipeFilterService.addMissingIngredients(ingredientList,
							lfvAdd);

					// lfv add ingredients if lfv pass

					if (lfvUpdatedIngredients.size() > ingredientList.size()) {

						lfvAddonRecipes.put("LFV_ADD_" + recipeData.get("recipe_id"),
								new Object[] { recipeData.get("recipe_id"), recipeData.get("recipe_name"),
										recipeData.get("recipe_category"), recipeData.get("food_category"),

										// Updated ingredients after adding missing items
										String.join(", ", lfvUpdatedIngredients),

										recipeData.get("preparation_time"), recipeData.get("cooking_time"),
										recipeData.get("tag"), recipeData.get("no_of_servings"),
										recipeData.get("cuisine_category"), recipeData.get("recipe_description"),
										recipeData.get("preparation_method"), recipeData.get("nutrient_values"),
                ...
                //LCHF add if lchfpass
         List<String> lchfUpdatedIngredients = recipeFilterService.addMissingIngredients( ingredientList,lchfAdd);
                
         if (lchfUpdatedIngredients.size() > ingredientList.size()) {

                    lchfAddonRecipes.put("LCHF_ADD_" + recipeData.get("recipe_id"),
                            new Object[] {
                                    recipeData.get("recipe_id"),
                                    recipeData.get("recipe_name"),
                                    recipeData.get("recipe_category"),
                                    ...
                                    String.join(", ", lchfUpdatedIngredients),
       ...
       // dbQueries.insertRow(conn, "lfv_recipes_addon", lfvAddonRecipes);
       // dbQueries.insertRow(conn, "lchf_recipes_addon", lchfAddonRecipes);
    }
				...
				String lchfPasss = recipeFilterService.passesEliminationStr(ingredientList, lchfEliminate);

				if (lchfPasss.equals("NOTFOUND")) {
